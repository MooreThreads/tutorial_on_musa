...
config_path = './data/bert-base-chinese/config.json'
checkpoint_path = './data/bert-base-chinese/pytorch_model.bin'
dict_path = './data/bert-base-chinese/vocab.txt'
device = 'musa' if torch.musa.is_available() else 'cpu'

torch.distributed.init_process_group(backend='mccl', world_size=world_size, rank=args.local_rank)
...
# 转换数据集
dataset = MyDataset('./data/china-people-daily-ner-corpus/example.train')
train_sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=world_size, rank=args.local_rank)
train_dataloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=4, sampler=train_sampler, collate_fn=collate_fn) 
valid_dataloader = DataLoader(MyDataset('./data/china-people-daily-ner-corpus/example.dev'), batch_size=batch_size, collate_fn=collate_fn) 
...
ddp_model = BaseModelDDP(model, master_rank=0, device_ids=[args.local_rank], output_device=args.local_rank)
class Loss(nn.Module):
    def forward(self, outputs, labels):
        return model.crf(*outputs, labels)

ddp_model.compile(loss=Loss(), optimizer=optim.Adam(model.parameters(), lr=4e-5), use_amp=True)
...
if __name__ == '__main__':
    ...
    evaluator = Evaluator()
    ddp_model.fit(train_dataloader, epochs=40, steps_per_epoch=None, callbacks=[evaluator])
