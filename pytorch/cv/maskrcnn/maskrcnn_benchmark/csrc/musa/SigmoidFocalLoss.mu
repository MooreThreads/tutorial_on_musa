...
at::Tensor SigmoidFocalLoss_forward_musa(
    ...
    const at::Tensor& targets,
		...
		const float gamma, 
		const float alpha) {
  TORCH_CHECK(logits.is_privateuseone(), "logits must be a MUSA tensor");
  TORCH_CHECK(targets.is_privateuseone(), "targets must be a MUSA tensor");
...
at::Tensor SigmoidFocalLoss_backward_musa(
  ...
  TORCH_CHECK(logits.is_privateuseone(), "logits must be a MUSA tensor");
  TORCH_CHECK(targets.is_privateuseone(), "targets must be a MUSA tensor");
  TORCH_CHECK(d_losses.is_privateuseone(), "d_losses must be a MUSA tensor");
